...
public class ChessBoardPart {
	...
	public void setFocus() {
		for(int r = Board.LENGTH - 1; r >= 0; --r) {					
			for(int c = 0; c < Board.LENGTH; ++c) {							
				if((r + c) % 2 == 0) {
					squares[r][c + 1].setBackground(Display.getDefault().getSystemColor(SWT.COLOR_DARK_MAGENTA));
				} else {
					squares[r][c + 1].setBackground(Display.getDefault().getSystemColor(SWT.COLOR_WHITE));
				}
				
				// Legal Moves indicates squares as Blue
				if(((Square) squares[r][c + 1].getData()).isLegal()) {
					Color lightBlue = new Color(Display.getDefault(), 145, 185, 255);
